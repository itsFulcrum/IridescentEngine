...
void main() {
	...
	// We are using a little trick here where we set 'first_instance' to our drawable_index so we can use it to fetch into matrix buffer 
	vec4 position_ws = _matrix_buffer[gl_InstanceIndex] * vec4(a_pos.xyz, 1.0f);
