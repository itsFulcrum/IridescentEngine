...
PrimitiveType_to_sdl_GPUPrimitiveType :: proc(type: PrimitiveType) -> sdl.GPUPrimitiveType {
	
	switch type {
		case PrimitiveType.TRIANGLELIST: 	return sdl.GPUPrimitiveType.TRIANGLELIST;
		case PrimitiveType.TRIANGLESTRIP: 	return sdl.GPUPrimitiveType.TRIANGLESTRIP;
		case PrimitiveType.LINELIST:		return sdl.GPUPrimitiveType.LINELIST;
		case PrimitiveType.LINESTRIP: 		return sdl.GPUPrimitiveType.LINESTRIP;
		case PrimitiveType.POINTLIST:		return sdl.GPUPrimitiveType.POINTLIST;
	}

	return sdl.GPUPrimitiveType.TRIANGLESTRIP;
}

DepthStencilFormat_to_sdl_GPUTextureFormat :: proc(format : DepthStencilFormat) -> sdl.GPUTextureFormat {
...
RenderTargetFormat_to_sdl_GPUTextureFormat :: proc(format : RenderTargetFormat, loc := #caller_location) -> sdl.GPUTextureFormat {

	switch format {
		case RenderTargetFormat.SWAPCHAIN: // this is kinda dirty.
			log.errorf("DONT: location: {}", loc);
			panic("Dont do this..")
			//return sdl.GPUTextureFormat.INVALID;
...
MSAA_to_sdl_GPUSampleCount :: proc(msaa_samples : MSAA) -> sdl.GPUSampleCount {
...
FillMode_to_sdl_GPUFillMode :: proc(fill_mode : FillMode) -> sdl.GPUFillMode {
...
CullMode_to_sdl_GPUCullMode :: proc(cull_mode : CullMode) -> sdl.GPUCullMode {
...
CompareOp_to_sdl_GPUCompareOp :: proc(compare_op : CompareOp) -> sdl.GPUCompareOp {
...
StencilOp_to_sdl_GPUStencilOp :: proc(stencil_op : StencilOp) -> sdl.GPUStencilOp {
...
BlendOp_to_sdl_GPUBlendOp :: proc(blend_op : BlendOp) -> sdl.GPUBlendOp {
...
BlendFactor_to_sdl_GPUBlendFactor :: proc(blend_factor : BlendFactor) -> sdl.GPUBlendFactor {
