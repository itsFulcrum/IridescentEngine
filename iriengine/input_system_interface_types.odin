...
// An Id that is return when registering callbacks and must be used to unregister callbacks.
InputID :: [2]i32
InputID_INVALID :: [2]i32{-1,-1}

// Callback Signatures
SDLEvent_CallbackSignature    			:: #type proc(sdl_event: ^sdl.Event);
MouseButton_CallbackSignature 			:: #type proc(mouse_pos: [2]f32, is_down : bool, is_double_click : bool);
...
GamepadDeviceEvent_CallbackSignature 	:: #type proc(device : GamepadDevice)
GamepadAnalog_CallbackSignature 		:: #type proc(device : GamepadDevice, value: [2]f32, delta: [2]f32)
GamepadButton_CallbackSignature 		:: #type proc(device : GamepadDevice, is_press: bool);



PressEventFlags :: bit_set[PressEvent]
PressEvent :: enum u8 {
	IsPressed, 		// Triggered if button is_down
	IsReleased, 	// Triggered if button is_up
	WasPressed,  	// Triggered if button went from is_up   to is_down
	WasReleased,	// Triggered if button went from is_down to is_up
}
...
InputCondition :: union { // 16 bit, atm.
	KeymodFlags,
	MouseButtonFlags,
	GamepadButtonFlags,
}

InputConditionType :: enum {
	None = 0,
	Keyboard,
	Mouse,
	Gamepad,
...
InputContext :: struct {
	...
	gamepad_device : GamepadDevice,
}


MouseButtonFlags :: bit_set[MouseButton; u8]
MouseButton :: enum u8 {
	LEFT 	= 0,
	MIDDLE 	= 1,
	RIGHT 	= 2,
...
// This enum mirrors 'sdl.KeymodFlag'
...
MAX_GAMEPADS :: len(GamepadDevice) - 1;
GamepadDevice :: enum u8 {
	GP_0 = 0,
	GP_1 = 1,
	GP_2 = 2,
	GP_3 = 3,
...
GamepadButtonFlags :: bit_set[GamepadButton; u32]
...
GamepadAnalogFlags :: bit_set[GamepadAnalog]
