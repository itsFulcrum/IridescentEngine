...
entity_get_entity_info :: proc(entity: Entity, universe: ^Universe = nil) -> (EntityInfo,EcsError) {
	uni := universe == nil ? get_active_universe() : universe;
	if uni == nil do return EntityInfo{}, EcsError.InvalidInputParameter;
	return ecs_get_entity_info(&uni.ecs, entity);
}


entity_rename :: proc(entity : Entity, new_name : string, universe: ^Universe = nil) -> EcsError{
	
	uni := universe == nil ? get_active_universe() : universe;
	if uni == nil do return EcsError.InvalidInputParameter;

	return ecs_entity_rename(&uni.ecs, entity, new_name);
}
...
entity_get_user_data :: proc(entity : Entity, universe: ^Universe = nil) -> (uintptr, EcsError) {
	...
	if !ecs_entity_exists(&uni.ecs, entity) {
		return 0, EcsError.InvalidEntity;
	}

	return uni.ecs.entity_infos[entity.id].user_data, EcsError.None;
}
...
entity_set_user_data :: proc(entity : Entity, data : uintptr, universe: ^Universe = nil) -> EcsError {
	...
	uni.ecs.entity_infos[entity.id].user_data = data;
